...
const ProjectsPro = () => {
  ...
  return (
    <>
      <section id="projects" ref={ref} className="py-24 bg-dark-800 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
          ...
          >
            {featuredProjects.map((project) => {
              ...
              return (
                ...
                >
                  <div className="relative p-8 md:p-10 rounded-3xl bg-dark-700/30 backdrop-blur-sm border border-dark-600/50 hover:border-dark-500/70 transition-all overflow-hidden">
                    {/* Gradient overlay */}
                    ...
                    <div className="relative">
                      <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
                        <div className="flex items-start gap-5">
                          <div className={`p-4 rounded-2xl bg-gradient-to-br ${project.color} bg-opacity-10 border border-white/5`}>
                      ...
                      <div className="flex flex-wrap items-center gap-3 mb-8">
                        {project.tags.map((tag) => (
                          <span
                            key={tag}
                            className="px-4 py-2 bg-dark-600/40 backdrop-blur-sm text-gray-300 rounded-full text-sm border border-dark-500/30"
                          >
                            {tag}
                          </span>
                        ))}
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        {project.metrics.map((metric) => (
                          <div key={metric.label} className="p-5 rounded-2xl bg-dark-600/30 backdrop-blur-sm border border-dark-500/30">
                            <div className={`text-3xl md:text-4xl font-bold bg-gradient-to-r ${project.color} bg-clip-text text-transparent mb-2`}>
                              {metric.value}
                            </div>
          ...
          <motion.div
            ...
            animate={isInView ? "visible" : "hidden"}
          >
            <h4 className="text-2xl font-bold text-white mb-6">Other Notable Projects</h4>
            <div className="grid md:grid-cols-2 gap-6">
              {otherProjects.map((project) => {
                ...
                return (
                  ...
                  >
                    <div className="h-full p-6 rounded-2xl bg-dark-700/30 backdrop-blur-sm border border-dark-600/50 hover:border-dark-500/70 transition-all">
                      <div className="flex items-start gap-4 mb-4">
                        <div className={`p-3 rounded-xl bg-gradient-to-br ${project.color} bg-opacity-10 border border-white/5`}>
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {project.tags.map((tag) => (
                          <span
                            key={tag}
                            className="px-3 py-1 bg-dark-600/40 text-gray-400 rounded-full text-xs border border-dark-500/30"
                          >
                            {tag}
                          </span>
                        ))}
        ...
        {selectedProject && selectedProject.featured && (
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
              className="bg-dark-700/50 backdrop-blur-xl border border-dark-600/50 rounded-3xl max-w-4xl w-full my-8"
            >
              <div className="p-8 md:p-12">
                ...
                <div className="space-y-8">
                  ...
                  <div className={`grid grid-cols-3 gap-6 pt-8 border-t border-dark-600/50`}>
                    {selectedProject.metrics.map((metric) => (
                      <div key={metric.label} className="text-center">
                        <div className={`text-3xl md:text-4xl font-bold bg-gradient-to-r ${selectedProject.color} bg-clip-text text-transparent mb-2`}>
                          {metric.value}
                        </div>
